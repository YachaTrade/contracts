...
contract UniswapV3ExternalAdapterTest is Test {
    ...
    MockERC20 wmon; // quote side (tokenIn in the vault flow)
    ...
    function setUp() public {
        wmon = new MockERC20("WMON", "WMON", 18);
        v1Meme = new MockERC20("V1MEME", "V1MEME", 18);
        (address token0, address token1) =
            address(wmon) < address(v1Meme) ? (address(wmon), address(v1Meme)) : (address(v1Meme), address(wmon));
        pool = new MockCapricornPool(token0, token1);
        wmon.mint(address(pool), POOL_LIQ);
    ...
    function test_swap_pushesAndSendsOutput() public {
        ...
        wmon.mint(address(adapter), amountIn); // push pattern: caller pre-funds the adapter

        uint256 amountOut = adapter.swap(address(pool), address(wmon), address(v1Meme), amountIn, address(this), "");

        assertEq(amountOut, 2 ether, "returns pool output");
        assertEq(v1Meme.balanceOf(address(this)), 2 ether, "recipient received tokenOut");
        assertEq(wmon.balanceOf(address(pool)), POOL_LIQ + amountIn, "pool received full input");
        assertEq(wmon.balanceOf(address(adapter)), 0, "adapter holds nothing at rest");
    }

    function test_swap_reverseDirection() public {
        // tokenIn = v1Meme covers the opposite zeroForOne branch of the WMON direction.
        uint256 amountIn = 3 ether;
        v1Meme.mint(address(adapter), amountIn);

        uint256 amountOut = adapter.swap(address(pool), address(v1Meme), address(wmon), amountIn, address(this), "");

        assertEq(amountOut, 3 ether); // rate 1:1
        assertEq(wmon.balanceOf(address(this)), 3 ether);
    ...
    function test_swap_uniswapV3CallbackSelector() public {
        ...
        uint256 amountIn = 1 ether;
        wmon.mint(address(adapter), amountIn);
    ...
    function test_swap_partialFill_refundsLeftoverToCaller() public {
        ...
        assertEq(wmon.balanceOf(address(this)), 1 ether, "unconsumed input refunded to caller");
        assertEq(wmon.balanceOf(address(pool)), POOL_LIQ + 1 ether, "pool only received the consumed half");
        assertEq(wmon.balanceOf(address(adapter)), 0, "adapter holds nothing at rest");
    }
    ...
    function test_swap_revertsOnZeroPool() public {
        wmon.mint(address(adapter), 1 ether);
        vm.expectRevert(UniswapV3ExternalAdapter.NoPair.selector);
        adapter.swap(address(0), address(wmon), address(v1Meme), 1 ether, address(this), "");
    }
    ...
    function test_swap_revertsOnTokenMismatch() public {
        ...
        adapter.swap(address(pool), address(wmon), address(other), 1 ether, address(this), "");
    }
    ...
    function test_callback_directSpoof_reverts() public {
        ...
        adapter.capricornCLSwapCallback(int256(1 ether), -int256(1 ether), abi.encode(address(wmon), uint256(1 ether)));

        vm.expectRevert(UniswapV3ExternalAdapter.NotExpectedPool.selector);
        adapter.uniswapV3SwapCallback(int256(1 ether), -int256(1 ether), abi.encode(address(wmon), uint256(1 ether)));

        vm.expectRevert(UniswapV3ExternalAdapter.NotExpectedPool.selector);
        adapter.pancakeV3SwapCallback(int256(1 ether), -int256(1 ether), abi.encode(address(wmon), uint256(1 ether)));
    }
    ...
    function test_swap_noCallback_reverts() public {
        ...
        adapter.swap(address(pool), address(wmon), address(v1Meme), 1 ether, address(this), "");
    }
    ...
    function test_swap_excessiveInput_reverts() public {
        ...
        adapter.swap(address(pool), address(wmon), address(v1Meme), amountIn, address(this), "");
    }
    ...
    function test_swap_reentrancy_blocked() public {
        ...
        pool.setReenterCall(
            abi.encodeCall(
                IDexAdapter.swap, (address(pool), address(wmon), address(v1Meme), 1, address(this), bytes(""))
            )
        );
        uint256 amountIn = 1 ether;
        wmon.mint(address(adapter), amountIn);
    ...
    function test_unsupportedFunctions_revert() public {
        ...
        adapter.getAmountOut(address(pool), address(wmon), 1 ether);
        vm.expectRevert(UniswapV3ExternalAdapter.NotSupported.selector);
        adapter.getAmountIn(address(pool), address(v1Meme), 1 ether);
        vm.expectRevert(UniswapV3ExternalAdapter.NotSupported.selector);
        adapter.addLiquidity(address(pool), address(wmon), address(v1Meme), 1, 1, address(this));
        vm.expectRevert(UniswapV3ExternalAdapter.NotSupported.selector);
