...
- **DividendVault 본체** — 비율 분할 기록(`pendingSwap`/`dividendBalance` 회계), bot 주도 변환
  ...
  (WNATIVE native unwrap 지원). 설계: `docs/plans/2026-06-12-dividend-bot-conversion-design.md` +
...
- **executeBondingBuy → executeConversion 내 router 직접 분기 통합** — `executeBondingBuy` 삭제,
  ...
  부적합. `router`는 `initialize` 파라미터로 유지(슬롯 시프트 없음). 일반 NadFunPair 풀(USDC↔WNATIVE,
