...
import {GIWA_WNATIVE} from "../normal/Deploy.s.sol";
...
contract DeployTreasurySafe is Script {
    function run() external {
        ...
        require(wnative.code.length > 0, "DeployTreasurySafe: canonical WNATIVE missing code");

        // -- 1. Deploy impl + proxy (deployer EOA) -------------------
        vm.startBroadcast(deployerKey);
        Treasury impl = new Treasury();
        ERC1967Proxy proxy =
            new ERC1967Proxy(address(impl), abi.encodeCall(Treasury.initialize, (protocolManager, wnative)));
        vm.stopBroadcast();
